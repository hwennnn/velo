...
class TripMember(SQLModel, table=True):
    ...
    trip_id: int = Field(foreign_key="trips.id", description="Trip this member belongs to")
    ...
    nickname: str = Field(description="Display name for this trip (can differ from user.display_name)")
    is_fictional: bool = Field(default=False, description="True if not linked to a real user")
    ...
    joined_at: Optional[datetime] = Field(
        default=None, 
        description="When a fictional member was claimed by a user"
    )
