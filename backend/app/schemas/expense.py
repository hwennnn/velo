...
class SplitCreate(BaseModel):
    ...
    amount: Optional[Decimal] = Field(None, ge=0, description="Amount owed (for custom split)")
    percentage: Optional[Decimal] = Field(None, ge=0, le=100, description="Percentage (for percentage split)")
...
class ExpenseCreate(BaseModel):
    ...
    category: Optional[str] = Field(None, max_length=50, description="Category (food, transport, accommodation, etc.)")
    notes: Optional[str] = Field(None, max_length=500, description="Additional notes")
    receipt_url: Optional[str] = Field(None, max_length=500, description="Receipt image URL")
    
    # Split information
    split_type: str = Field(default="equal", description="Split type: equal, percentage, custom")
    splits: Optional[list[SplitCreate]] = Field(None, description="Custom/percentage splits")
    ...
    def validate_expense_data(self):
        ...
        if self.split_type in ["percentage", "custom"]:
            if not self.splits or len(self.splits) == 0:
                raise ValueError(f'Splits list is required for {self.split_type} split type')
        ...
        if self.split_type == "custom" and self.splits:
            ...
            if abs(total_amount - self.amount) > 0.01:  # Allow small floating point errors
                raise ValueError(f'Custom split amounts must sum to expense amount {self.amount}, got {total_amount}')
