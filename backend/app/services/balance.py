...
class Settlement:
    ...
    def __init__(self, from_member_id: int, to_member_id: int, amount: Decimal,
                 from_nickname: str = "", to_nickname: str = ""):
...
def calculate_balances(trip_id: int, session: Session) -> List[Balance]:
    """
...
def calculate_settlements(trip_id: int, session: Session) -> List[Settlement]:
    """
...
def get_member_balance_details(trip_id: int, member_id: int, session: Session) -> Dict:
    """
    ...
    paid_expenses = session.exec(paid_statement).all()
    ...
    splits_with_expenses = session.exec(splits_statement).all()
